"""Single-purpose invoice: restock Nutella 400g by 100 units.

The line item name "Nutella 400g" fuzzy-matches the seeded product exactly
(score 100 >= 85), so on upload it adds +100 to that product's stock
(6 -> 106), clearing its LOW reorder alert. Unit price $3.10 matches the last
seeded invoice, so NO price-increase alert is raised and cost/margin are
unchanged. Supplier is the seeded "Cedar Wholesale Trading" to keep history clean.

Output: test_invoices/play/nutella_restock_100.pdf
"""
import os
...
path = os.path.join(OUT, "nutella_restock_100.pdf")
...
c.setFillColorRGB(0.4, 0.4, 0.4)
c.drawRightString(W - 2 * cm, H - 2.7 * cm, "Invoice #:  CWT-2026-0461")
c.drawRightString(W - 2 * cm, H - 3.1 * cm, "Date:         2026-06-16")
c.drawRightString(W - 2 * cm, H - 3.5 * cm, "Due Date:  2026-07-01")
c.setFillColorRGB(0, 0, 0)
line(c, H - 3.9 * cm)
...
c.drawString(2.3 * cm, y - 0.2 * cm, "Nutella 400g")
c.drawString(10 * cm, y - 0.2 * cm, str(qty))
c.drawString(12.5 * cm, y - 0.2 * cm, "jar")
