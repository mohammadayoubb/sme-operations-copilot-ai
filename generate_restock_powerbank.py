"""Restock Power Bank 10000mAh by 100 units.

Line item "Power Bank 10000mAh" matches the seeded product exactly (score 100 >= 85),
so on upload it adds +100 to stock (0 -> 100), clearing the CRITICAL alert.
Unit price $15.00 matches the seed cost, no price-increase alert.
"""
import os
...
path = os.path.join(OUT, "powerbank_restock_100.pdf")
...
c.setFillColorRGB(0.4, 0.4, 0.4)
c.drawRightString(W - 2 * cm, H - 2.7 * cm, "Invoice #:  CWT-2026-0462")
...
c.drawString(2.3 * cm, y - 0.2 * cm, "Power Bank 10000mAh")
