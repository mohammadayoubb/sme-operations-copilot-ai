...
Targets the souk_tyre business (searches by name "tyre" → "souk" → first).
...
def _daily_qty(cfg: dict, d: date) -> int:
    ...
    if anomaly and d >= _ANOMALY_START:
        ...
        if anomaly == "spike":
            return max(0, round(cfg["base"] * 4 + random.gauss(0, cfg["noise"])))  # 4× → z > +2
...
def main():
    ...
    try:
        ...
        # Find souk_tyre by name (tyre → souk → first)
        business = db.execute(
        ...
        if business is None:
            raise RuntimeError("No business found — has the app been initialised?")
